...
const addBird = async (req, res, next) => {
  ...
  if (!dUser) {
    const error = new HttpError("Colud not find user for that id", 404);
    return next(error);
  }
  ...
  try {
    ...
    // await newBird.save({ session: session });
    // dUser.birds.push(newBird);
    // await dUser.save({ session: session });
...
const deleteBird = async (req, res, next) => {
  ...
  try {
    birdsData = await BirdSchema.findOneAndDelete(birdId); //.populate("user");
    user = await userSchema.findById(birdsData.user);
    //console.log(birdsData, user.birds);
  } catch (err) {
    const error = new HttpError("Failed", 500);
    return next(error);
  }
